...
object LudoRules {
    ...
    /** Pions qu'un joueur peut bouger avec ce dé. */
    fun playable(player: Player, dice: Int): List<Piece> =
        player.pieces.filter { p ->
            !p.arrived && (if (p.atBase) dice == 6 else newPos(p, dice) != null)
        }

    /** Vérifie si une case est une forteresse (2+ pions même couleur). */
    fun isFortress(pos: Int, players: List<Player>): Boolean {
        if (pos < 0 || pos in Board.SAFE) return false
        return players.any { pl -> pl.pieces.count { it.pos == pos } >= 2 }
    }

...
fun GameScreen(
    ...
    val pColor  = player?.color?.base() ?: Color(0xFFFFD700)
    ...
    Box(
        ...
        Column(
            ...
            GameTopBar(state.current, state.players, onQuit)
...
fun PodiumScreen(
    ...
    Box(
        ...
        Column(
            ...
            Text("🏆", fontSize = 60.sp)
            Text("FIN DE PARTIE", fontSize = 22.sp, fontWeight = FontWeight.ExtraBold, color = Color(0xFF333333))
            ...
            if (isRec) {
                Surface(
                    ...
                    Text("🏅 NOUVEAU RECORD !", Modifier.padding(horizontal = 16.dp, vertical = 6.dp),
                         fontWeight = FontWeight.ExtraBold, color = Color(0xFFFF8F00))
                }
            }
            ...
            if (hasPrize) {
                Surface(
                    ...
                    Column(Modifier.padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                        Text("💰 PRIZE POOL", fontSize = 11.sp, color = Color(0xFF888888), letterSpacing = 1.sp)
                        Text(formatCDF(prizePool), fontSize = 24.sp, fontWeight = FontWeight.ExtraBold, color = Color(0xFFFF8F00))
                    }
                }
            }
            ...
            val medals = listOf("🥈", "🥇", "🥉")
            val barColors = listOf(Color(0xFFC0C0C0), Color(0xFFFFD700), Color(0xFFCD7F32))

            Row(Modifier.fillMaxWidth().height(210.dp),
                horizontalArrangement = Arrangement.spacedBy(10.dp, Alignment.CenterHorizontally),
                verticalAlignment = Alignment.Bottom) {
                podOrder.forEachIndexed { vi, pl ->
                    val h by animateDpAsState(podHeights[vi],
                        spring(Spring.DampingRatioMediumBouncy, Spring.StiffnessLow), label = "h$vi")
                    Column(Modifier.width(88.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                        Text(pl.color.emoji, fontSize = 22.sp)
                        Text(pl.name.take(8), fontWeight = FontWeight.Bold, fontSize = 11.sp, color = pl.color.base(), maxLines = 1)
                        Text(medals[vi], fontSize = 20.sp)
                        Box(Modifier.fillMaxWidth().height(h)
                            .clip(RoundedCornerShape(topStart = 8.dp, topEnd = 8.dp))
                            .background(barColors[vi].copy(alpha = 0.85f)), Alignment.TopCenter) {
                            Text("#${pl.rank}", Modifier.padding(top = 6.dp), fontWeight = FontWeight.Bold, fontSize = 14.sp, color = Color.White.copy(alpha = 0.85f))
                        }
                    }
                }
            }

            Surface(Modifier.fillMaxWidth(), RoundedCornerShape(14.dp), Color.White, BorderStroke(1.dp, Color(0xFFEEEEEE))) {
                Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("📊 STATISTIQUES", fontSize = 11.sp, color = Color(0xFF888888), letterSpacing = 1.sp, fontWeight = FontWeight.Bold)
                    Row(Modifier.fillMaxWidth()) {
                        listOf("Joueur", "✅", "✕", "☠").forEach { header ->
                            Text(header, Modifier.weight(1f), fontSize = 10.sp, color = Color(0xFFAAAAAA), textAlign = TextAlign.Center)
                        }
                    }
                    HorizontalDivider(color = Color(0xFFEEEEEE))
                    sorted.forEach { pl ->
                        Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                            Text("${pl.color.emoji} ${pl.name.take(7)}", Modifier.weight(1f), fontSize = 10.sp, color = pl.color.base())
                            listOf("${pl.doneCount}/4", "${pl.captures}", "${pl.suffered}").forEach { v ->
                                Text(v, Modifier.weight(1f), fontSize = 10.sp, color = Color(0xFF333333), textAlign = TextAlign.Center)
                            }
                        }
                    }
                }
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                Button(onClick = onReplay, modifier = Modifier.weight(1f).height(50.dp),
                       shape = RoundedCornerShape(12.dp),
                       colors = ButtonDefaults.buttonColors(containerColor = Color(0xFF4CAF50), contentColor = Color.White)) {
                    Text("🔄  Rejouer", fontWeight = FontWeight.Bold)
                }
                OutlinedButton(onClick = onMenu, modifier = Modifier.weight(1f).height(50.dp),
                               shape = RoundedCornerShape(12.dp), border = BorderStroke(1.dp, Color(0xFFCCCCCC)),

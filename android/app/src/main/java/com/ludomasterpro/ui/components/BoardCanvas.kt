...
// ══════════════════════════════════════════════════════════════
//  BoardCanvas — Style Ludo Master
//  • Pions quille 3D (tête ronde + corps évasé + reflets)
//  • Cellules blanc/gris, zones maison colorées + rect blanc
//  • Étoiles ★ sur cases sûres
//  • Couloirs colorés, centre 4 triangles
...
fun BoardCanvas(
    ...
    Canvas(
            ...
            .pointerInput(playableIds) {
                detectTapGestures { tap ->
                    ...
                    for (p in pieces) {
                        ...
                        val dist = (tap - Offset(c * cell + cell / 2f + dx,
                                                  r * cell + cell / 2f + dy)).getDistance()
        ...
        for ((col, org) in homeCorners) {
            ...
            // Rectangle blanc intérieur (espace des pions)
            val mg  = cell * 0.44f
            val iSz = s - mg * 2f
            drawRoundRect(
        ...
        for ((idx, rc) in Board.PATH.withIndex()) {
            ...
            // Fond
            drawRect(bg,     Offset(x + pad, y + pad), Size(cell - pad * 2, cell - pad * 2))
            // Bordure grise
            drawRect(
                Color(0xFFCCCCCC),
                Offset(x + pad, y + pad),
            ...
            if (idx in Board.SAFE) {
                ...
                drawText(sm, topLeft = Offset(cx(c) - sm.size.width / 2f,
        ...
        val mid = Offset(cx(Board.CENTER.second), cy(Board.CENTER.first))
        val hs  = cell * 0.5f

        data class Tri(val color: Color, val p1: Offset, val p2: Offset)
        listOf(
            Tri(PieceColor.GREEN.base(),  Offset(mid.x - hs, mid.y - hs), Offset(mid.x + hs, mid.y - hs)),
            Tri(PieceColor.BLUE.base(),   Offset(mid.x - hs, mid.y - hs), Offset(mid.x - hs, mid.y + hs)),
            Tri(PieceColor.RED.base(),    Offset(mid.x - hs, mid.y + hs), Offset(mid.x + hs, mid.y + hs)),
            Tri(PieceColor.YELLOW.base(), Offset(mid.x + hs, mid.y - hs), Offset(mid.x + hs, mid.y + hs)),
        ).forEach { t ->
            drawPath(
                Path().apply {
                    moveTo(mid.x, mid.y)
        ...
        // Étoile blanche au centre
        drawCircle(Color.White, cell * 0.38f, mid)
        val smC = tm.measure(
            AnnotatedString("★"),
            style = TextStyle(
        ...
        data class Arrow(val row: Int, val col: Int, val char: String, val color: Color)
        listOf(
            Arrow(0,  7,  "∨", PieceColor.GREEN.base()),
            Arrow(14, 7,  "∧", PieceColor.RED.base()),
            Arrow(7,  0,  ">", PieceColor.YELLOW.base()),
            Arrow(7,  14, "<", PieceColor.BLUE.base()),
        ).forEach { a ->
            val am = tm.measure(
            ...
            drawText(am, topLeft = Offset(cx(a.col) - am.size.width / 2f,
...
private fun pawnOff(piece: Piece, cell: Float): Pair<Float, Float> {
    ...
    val offsets = arrayOf(-f to -f, f to -f, -f to f, f to f)
    val (a, b) = offsets[piece.index]
    return a * cell to b * cell
}
...
private fun DrawScope.drawPawn(
    ...
    val bW   = h * 0.24f   // demi-largeur max
    ...
    // Corps — dégradé gauche→droite
    drawPath(
        bodyPath,
        brush = Brush.linearGradient(
            colors = listOf(light.copy(0.95f), base, dark),
            start  = Offset(center.x - bW, bTop),
            end    = Offset(center.x + bW, bBot)
        )
    )
    drawPath(bodyPath, color = dark, style = Stroke(width = 1.3f))

    // Tête — cercle avec dégradé radial 3D
    drawCircle(
    ...
    drawCircle(dark, headR, Offset(center.x, headY), style = Stroke(1.3f))

    // Reflet brillant sur la tête (haut-gauche)
    drawOval(
